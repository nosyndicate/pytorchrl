...
AWS_S3_PATH = 's3://$s3_bucket_name/pytorchrl/experiments'

AWS_CODE_SYNC_S3_PATH = 's3://$s3_bucket_name/pytorchrl/code'
...
def setup_iam():
    """
    Set up IAM (Identity and Access Management) Roles.
    Application must sign their API request with AWS credentials.
    Thus, we should securely distribute the AWS credentials to the instances.

    IAM roles are designed so that  applications can securely make API requests
    from instances, without requiring to manage the security credentials that
    the applications use. Instead of creating and distributing AWS credentials,
    we can delegate permission to make API requests using IAM roles

    IAM roles can be used to grant permissions to applications running on instances
    that needs to use a bucket in Amazon S3. We can specify permissions for IAM
    roles by creating a policy in JSON format.
    """
    ...
    try:
        existing_role = iam.Role('pytorchrl')
        existing_role.load()
        ...
        # If we use the AWS CLI, API, or an AWS SDK to launch an instance with
        # an IAM role or to attach an IAM role to an instance, we need to specify
    ...
    print('Creating instance profile pytorchrl')
    iam_client.create_instance_profile(
        InstanceProfileName='pytorchrl',
        Path='/'
    )
    print('Adding role pytorchrl to instance profile pytorchrl')
...
def setup_ec2():
    for region in ['us-east-1', 'us-east-2']:
        ...
        # Every one has a VPC (private cloud), to easily manage the ip address
        ...
        try:
            security_group = vpc.create_security_group(
                GroupName='pytorchrl-sg', Description='Security group for pytorchrl'
            )
        except botocore.exceptions.ClientError as e:
            if e.response['Error']['Code'] == 'InvalidGroup.Duplicate':
                sgs = list(vpc.security_groups.filter(GroupNames=['pytorchrl-sg']))
        ...
        ec2_client.create_tags(Resources=[security_group.id], Tags=[{'Key': 'Name', 'Value': 'pytorchrl-sg'}])
        try:
...
def write_config():
    ...
    config_personal_file = os.path.join(config.PROJECT_PATH, 'pytorchrl/config_personal.py')
    if os.path.exists(config_personal_file):
        if not query_yes_no('pytorchrl/config_personal.py exists. Override?', 'no'):
            sys.exit()
